...
# Taken from: https://github.com/killfrenzy96/KatOscApp/blob/main/katosc.py
...
class KatOscConfig:
    def __init__(self):
        ...
        # Used to improve sync with in-game avatar and autodetect sync parameter count used for the avatar.
        self.osc_enable_server: bool = True
        ...
        # Delay between network updates in seconds. Setting this too low will cause issues.
        self.osc_delay: float = 0.25
        # Default sync parameters. This is automatically updated if the OSC server is enabled.
        self.sync_params: int = 4

        self.line_length: int = 32  # Characters per line of text
        self.line_count: int = 4  # Maximum lines of text

        # Send a copy of the text to the VRChat chat box
        self.osc_enable_chatbox: bool = False
        # Delay between chatbox updates in seconds. Setting this too low will cause issues.
        self.osc_chatbox_delay: float = 1.25


class KatOsc:
    def __init__(self, osc_client: SimpleUDPClient, config: KatOscConfig = KatOscConfig()):
        ...
        # Character value to use when testing sync parameters
        self.sync_params_test_char_value: int = 97

        self.param_visible: str = "osc_discord_KAT_Visible"
        self.param_pointer: str = "osc_discord_KAT_Pointer"
        self.param_sync: str = "osc_discord_KAT_CharSync"
        ...
        # Setup OSC Chatbox
        self.osc_chatbox_timer = RepeatedTimer(
            self.osc_chatbox_delay, self.osc_chatbox_loop)
        ...
        self.osc_client.send_message(
            self.osc_parameter_prefix + self.param_visible, True)  # Make KAT visible
        self.osc_client.send_message(
            self.osc_parameter_prefix + self.param_pointer, 255)  # Clear KAT text
            ...
            if self.osc_server_test_step > 0:
                ...
                if self.osc_server_test_step == 1:
                    ...
                    for char_index in range(self.sync_params_max):
                        self.osc_client.send_message(
                            self.osc_parameter_prefix + self.param_sync + str(char_index), 0.0)
                    ...
                    for char_index in range(self.sync_params_max):
                        self.osc_client.send_message(self.osc_parameter_prefix + self.param_sync + str(
                            char_index), self.sync_params_test_char_value / 127.0)
                    ...
                    if self.sync_params == 0:
                        # Test failed, reuse last detected param count
                        self.sync_params = self.sync_params_last
                    else:
                        self.sync_params_last = self.sync_params
                    self.osc_server_test_step = 0
                    self.pointer_count = int(
                        self.text_length / self.sync_params)
                    self.osc_text = " ".ljust(
                        self.text_length)  # Resync letters
    ...
    def osc_update_pointer(self, pointer_index: int, gui_text: str, osc_chars: list[int]):
        self.osc_client.send_message(
            self.osc_parameter_prefix + self.param_pointer, pointer_index + 1)  # Set pointer position
        ...
        for char_index in range(self.sync_params):
            ...
            self.osc_client.send_message(
                self.osc_parameter_prefix + self.param_sync + str(char_index), value)
    ...
    def show(self):
        self.osc_client.send_message(
            self.osc_parameter_prefix + self.param_visible, True)  # Hide KAT

    # hide overlay

    def hide(self):
        self.osc_client.send_message(
            self.osc_parameter_prefix + self.param_visible, False)  # Hide KAT
